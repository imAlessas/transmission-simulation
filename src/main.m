...
DEBUG = 1;      % Set to 1 to enable additional debugging display
...
% Additional data
transmitted_symbol_number = randi(100,1);     % number of symbols   
...
sliced_disturbed_signal = reshape(BPSK_signal, samples_per_symbol, N);
...
fprintf("Errors occurred during the transmission:\n %i\n", errors_occurred);
...
sequence = randi(2,1, 800) - 1

padded_sequence = add_padding_bits(sequence)

padded_matrix = reshape(padded_sequence, k, length(padded_sequence)/k)';

coded_matrix = hamming_encoding(padded_matrix, codeword_length, k, generation_polynomial);

coded_matrix = coded_matrix';

coded_sequence = coded_matrix(:)';

coded_matrix = reshape(coded_sequence, codeword_length, length(coded_sequence) / codeword_length)';

decoded_matrix = hamming_decoding(coded_matrix, codeword_length, k, generation_polynomial);

decoded_matrix = decoded_matrix';

decoded_sequence = decoded_matrix(:)';



sum(decoded_sequence ~= padded_sequence)
